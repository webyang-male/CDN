    ...
    !(function () {
        ...
        function d3_geo_areaRingStart() {
          ...
          d3_geo_area.point = function (位, 蠁) {
            ...
            (位0 = (位00 = 位) * d3_radians),
              (cos蠁0 = Math.cos((蠁 = ((蠁00 = 蠁) * d3_radians) / 2 + 蟺 / 4))),
              (sin蠁0 = Math.sin(蠁));
          };
          ...
          return function (feature) {
            ...
            if (n) {
              ...
              for (
                ...
                b = merged[i];
                if ((d位 = angle(a[1], b[0])) > best)
                  (best = d位), (位0 = b[0]), (位1 = a[1]);
              }
            }
            ranges = range = null;
        ...
        function d3_geo_clip(pointVisible, clipLine, interpolate, clipStart) {
          return function (rotate, listener) {
            ...
            function point(位, 蠁) {
              ...
              if (pointVisible((位 = point[0]), (蠁 = point[1])))
                listener.point(位, 蠁);
            }
        ...
        function d3_geo_clipSort(a, b) {
          return (
            ((a = a.x)[0] < 0 ? a[1] - half蟺 - 蔚 : half蟺 - a[1]) -
            ((b = b.x)[0] < 0 ? b[1] - half蟺 - 蔚 : half蟺 - b[1])
          );
        }
        function d3_geo_pointInPolygon(point, polygon) {
          ...
          for (var i = 0, n = polygon.length; i < n; ++i) {
            ...
            while (true) {
              ...
              if (antimeridian ^ (位0 >= meridian) ^ (位 >= meridian)) {
                ...
                var 蠁arc =
                  (antimeridian ^ (d位 >= 0) ? -1 : 1) * d3_asin(intersection[2]);
          ...
          return (
            (polarAngle < -蔚 || (polarAngle < 蔚 && d3_geo_areaRingSum < 0)) ^
            (winding & 1)
          );
        }
        ...
        function d3_geo_clipAntimeridianIntersect(位0, 蠁0, 位1, 蠁1) {
          var cos蠁0,
            cos蠁1,
            sin位0_位1 = Math.sin(位0 - 位1);
          return abs(sin位0_位1) > 蔚
            ? Math.atan(
                (Math.sin(蠁0) * (cos蠁1 = Math.cos(蠁1)) * Math.sin(位1) -
                  Math.sin(蠁1) * (cos蠁0 = Math.cos(蠁0)) * Math.sin(位0)) /
                  (cos蠁0 * cos蠁1 * sin位0_位1)
              )
            : (蠁0 + 蠁1) / 2;
        }
        ...
        function d3_geo_clipCircle(radius) {
          ...
          return d3_geo_clip(
            ...
            clipLine,
            interpolate,
            smallRadius ? [0, -radius] : [-蟺, radius - 蟺]
          );
          ...
          function intersect(a, b, two) {
            ...
            if (
              meridian
                ? polar
                  ? (蠁0 + 蠁1 > 0) ^ (q[1] < (abs(q[0] - 位0) < 蔚 ? 蠁0 : 蠁1))
                  : 蠁0 <= q[1] && q[1] <= 蠁1
                : (未位 > 蟺) ^ (位0 <= q[0] && q[0] <= 位1)
            ) {
        ...
        function d3_geo_conicEqualArea(蠁0, 蠁1) {
          ...
          forward.invert = function (x, y) {
            ...
            return [
              Math.atan2(x, 蟻0_y) / n,
              d3_asin((C - (x * x + 蟻0_y * 蟻0_y) * n * n) / (2 * n)),
            ];
          };
          return forward;
        }
        ...
        d3.geo.albersUsa = function () {
          ...
          albersUsa.translate = function (_) {
            ...
            alaskaPoint = alaska
              ...
              .clipExtent([
                [x - 0.425 * k + 蔚, y + 0.12 * k + 蔚],
                [x - 0.214 * k - 蔚, y + 0.234 * k - 蔚],
              ])
              .stream(pointStream).point;
            hawaiiPoint = hawaii
              .translate([x - 0.205 * k, y + 0.212 * k])
              .clipExtent([
                [x - 0.214 * k + 蔚, y + 0.166 * k + 蔚],
                [x - 0.115 * k - 蔚, y + 0.234 * k - 蔚],
              ])
              .stream(pointStream).point;
            return albersUsa;
          };
        ...
        function d3_geo_projectionMutator(projectAt) {
          ...
          projection.rotate = function (_) {
            if (!arguments.length)
              return [未位 * d3_degrees, 未蠁 * d3_degrees, 未纬 * d3_degrees];
          ...
          function reset() {
            projectRotate = d3_geo_compose(
              (rotate = d3_geo_rotation(未位, 未蠁, 未纬)),
              project
            );
        ...
        function d3_geo_rotation蠁纬(未蠁, 未纬) {
          ...
          function rotation(位, 蠁) {
            var cos蠁 = Math.cos(蠁),
              ...
              k = z * cos未蠁 + x * sin未蠁;
            return [
              Math.atan2(y * cos未纬 - k * sin未纬, x * cos未蠁 - z * sin未蠁),
              d3_asin(k * cos未纬 + y * sin未纬),
            ];
          }
          rotation.invert = function (位, 蠁) {
            var cos蠁 = Math.cos(蠁),
              ...
              k = z * cos未纬 - y * sin未纬;
            return [
              Math.atan2(y * cos未纬 + z * sin未纬, x * cos未蠁 + k * sin未蠁),
              d3_asin(k * cos未蠁 - x * sin未蠁),
            ];
          };
          return rotation;
        }
        ...
        d3.geo.distance = function (a, b) {
          ...
          return Math.atan2(
            Math.sqrt(
              (t = cos蠁1 * sin螖位) * t +
                (t = cos蠁0 * sin蠁1 - sin蠁0 * cos蠁1 * cos螖位) * t
            ),
        ...
        function d3_geo_lengthLineStart() {
          ...
          function nextPoint(位, 蠁) {
            ...
            d3_geo_lengthSum += Math.atan2(
              Math.sqrt(
                (t = cos蠁 * Math.sin(t)) * t +
                  (t = cos蠁0 * sin蠁 - sin蠁0 * cos蠁 * cos螖位) * t
              ),
              sin蠁0 * sin蠁 + cos蠁0 * cos蠁 * cos螖位
            );
        ...
        function d3_geo_azimuthal(scale, angle) {
          ...
          azimuthal.invert = function (x, y) {
            var 蟻 = Math.sqrt(x * x + y * y),
              ...
              cosc = Math.cos(c);
            return [
              Math.atan2(x * sinc, 蟻 * cosc),
              Math.asin(蟻 && (y * sinc) / 蟻),
            ];
          };
          return azimuthal;
        }
        ...
        function d3_geo_conicConformal(蠁0, 蠁1) {
          var cos蠁0 = Math.cos(蠁0),
            ...
            n =
              蠁0 === 蠁1
                ? Math.sin(蠁0)
                : Math.log(cos蠁0 / Math.cos(蠁1)) / Math.log(t(蠁1) / t(蠁0)),
          ...
          forward.invert = function (x, y) {
            ...
            return [
              Math.atan2(x, 蟻0_y) / n,
              2 * Math.atan(Math.pow(F / 蟻, 1 / n)) - half蟺,
            ];
          };
          return forward;
        }
        ...
        function d3_geo_conicEquidistant(蠁0, 蠁1) {
          ...
          forward.invert = function (x, y) {
            ...
            return [
              Math.atan2(x, 蟻0_y) / n,
              G - d3_sgn(n) * Math.sqrt(x * x + 蟻0_y * 蟻0_y),
            ];
          };
          return forward;
        }
        ...
        function d3_geom_voronoiRemoveBeach(beach) {
          ...
          while (
            lArc.circle &&
            abs(x - lArc.circle.x) < 蔚 &&
            abs(y - lArc.circle.cy) < 蔚
          ) {
            previous = lArc.P;
          ...
          while (
            rArc.circle &&
            abs(x - rArc.circle.x) < 蔚 &&
            abs(y - rArc.circle.cy) < 蔚
          ) {
            next = rArc.N;
        ...
        function d3_geom_voronoiClipEdges(extent) {
          ...
          while (i--) {
            ...
            if (
              ...
              !clip(e) ||
              (abs(e.a.x - e.b.x) < 蔚 && abs(e.a.y - e.b.y) < 蔚)
            ) {
              e.a = e.b = null;
              edges.splice(i, 1);
            }
